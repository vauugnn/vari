...
// Export the session's command log as an SPSS syntax file. Paired with the
// saved .sav, a professor can run this in IBM SPSS to reproduce the analysis as
// native SPSS output — the openable-in-SPSS path (Vari commands ARE SPSS syntax).
async function exportSyntax(): Promise<void> {
  const de = windows.dataeditor ?? undefined
  let body = ''
  try {
    body = readFileSync(journalPath(), 'utf8')
  } catch {
    body = ''
  }
  const res = await dialog.showSaveDialog(de as BrowserWindow, {
    title: 'Export SPSS Syntax',
    defaultPath: 'analysis.sps',
    filters: [{ name: 'SPSS Syntax (*.sps)', extensions: ['sps'] }]
  })
  if (res.canceled || !res.filePath) return
  const header =
    '* Generated by Vari. Open your data (.sav) in IBM SPSS Statistics, then\n' +
    '* Run > All to reproduce this analysis as native SPSS output.\n\n'
  await writeFile(res.filePath, header + body, 'utf8')
}
...
app.whenReady().then(() => {
  Menu.setApplicationMenu(
    buildMenu({
      ...
      exportSyntax: () => void exportSyntax(),
